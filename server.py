...
# ──────────────────────── INIT OCR ENGINE ────────────────────────
# Initialize RapidOCR once at startup so it loads models into memory
try:
    ocr_engine = RapidOCR()
...
app = FastAPI(title="Clean OCR Summarizer", version="8.0")
...
def extract_text_with_rapidocr(raw_bytes: bytes) -> str:
    """
    Uses PyMuPDF for text and RapidOCR (Python Lib) for images.
    """
    ...
    for page_num, page in enumerate(doc):
        ...
        # 2. Image OCR (Clean Python Library)
        if ENABLE_OCR and ocr_engine:
            try:
                ...
                for i, img in enumerate(image_list):
                    ...
                    # RapidOCR works with image bytes or paths. 
                    # It usually prefers bytes or numpy, let's pass bytes directly if supported
                    # or use PIL to open it.
                    
                    result, elapse = ocr_engine(image_bytes)
                    
                    if result:
                        # result is list of tuples: [[ [x1,y1], [x2,y2], text, confidence ]]
...
async def call_llm(prompt: str, system_prompt: str, is_retry: bool = False, temp: float = 0.4) -> str:
    ...
    for client in client_pool:
        for model in models:
            if not breaker.is_available(client): continue
...
async def process_pdf(request: Request, file: UploadFile = File(...), mode: str = Form(...)):
    try:
        ...
        # Using the new clean extraction function
