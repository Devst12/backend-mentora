...
app = FastAPI(title="Final Summarizer", version="6.0")
...
def extract_text_from_pdf(raw_bytes: bytes) -> str:
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail="Corrupt PDF.")
    return "\n".join(text)
            ...
            try:
                async with sem:
                    response = await run_in_threadpool(
                        ...
                        model=model,
                        temperature=temp, # Allow randomness control
...
async def safe_summarize_task(chunk: str) -> Optional[str]:
    try:
        ...
        return await call_llm(prompt, sys_prompt, is_retry=False, temp=0.3)
    except Exception as e:
        try:
            return await call_llm(prompt, sys_prompt, is_retry=True, temp=0.3)
        except:
            return None

async def get_master_summary(chunks: List[str]) -> str:
    """Analyzes whole PDF by summarizing chunks then combining."""
    ...
    final_prompt = "Combine these notes into one cohesive summary covering the entire document:\n" + combined_text
...
async def validate_and_fix_json(json_str: str) -> str:
    ...
    try:
        ...
        logger.warning("Invalid JSON. Fixing...")
        ...
        try:
            ...
            raise ValueError("AI failed to fix JSON.")

async def generate_mcq_logic(chunks: List[str]) -> str:
    """
    Generates MCQs based on the MASTER SUMMARY of the whole PDF.
    """
    logger.info("Generating Master Summary for Quiz...")
    master_summary = await get_master_summary(chunks)
    
    logger.info("Generating 10 MCQs from Master Summary...")
    system_prompt = """
    You are a JSON API. Generate 10 DIFFERENT Multiple Choice Questions based on the text.
    ...
    try:
        raw_response = await call_llm(
            f"Text: {master_summary[:8000]}", # Limit summary size for final prompt
            system_prompt, 
            is_retry=False, 
            temp=0.8 # High temp for variety
        )
...
async def generate_summary_logic(chunks: List[str], is_long: bool) -> str:
    ...
    return await call_llm(final_prompt, sys_prompt, is_retry=False, temp=0.4)

@app.post("/process_pdf")
async def process_pdf(request: Request, file: UploadFile = File(...), mode: str = Form(...)):
    try:
        raw_bytes = await file.read()
        validate_pdf(file, raw_bytes)
        text = await run_in_threadpool(extract_text_from_pdf, raw_bytes)
        if not text.strip():
